...
{
    DatabaseManager *dataManager = [[DatabaseManager alloc] init];
    [dataManager autorelease];
    return [dataManager databasePathWithDatabaseName:databaseName];
    
}

-(BOOL)connectionDatabaseName:(NSString *)databaseName tableName:(NSString *)tableName sqlString:(NSString *)sql
{
    NSString *dbPath = [DatabaseManager datamanagerDatabasePathWithDatabaseName:databaseName];
    NSLog(@"数据库路径 ： %@", dbPath );
...
{
    DatabaseManager *databaseManager = [[[DatabaseManager alloc] init] autorelease];
    BOOL result = [databaseManager connectionDatabaseName:databaseName tableName:tableName sqlString:sql];
...
- (BOOL)useDatabase:(NSString *)databaseName deleteFromTable:(NSString *)tableName whereColumn1:(NSString *)column1 value1:(NSString *)value1 andColumn2:(NSString *)column2 value2:(NSString *)value2
{
    NSString *dbPath = [DatabaseManager datamanagerDatabasePathWithDatabaseName:databaseName];
    NSLog(@"数据库地址 %@", dbPath);
    FMDatabase *database = [FMDatabase databaseWithPath:dbPath];
...
- (NSMutableArray *)useDatabase:(NSString *)databaseName selectAllFromTable:(NSString *)tableName whereColumn1:(NSString *)column1 value1:(NSString *)value1 column2:(NSString *)column2 value2:(NSString *)value2
{
    NSString *dbPath = [DatabaseManager datamanagerDatabasePathWithDatabaseName:databaseName];
    FMDatabase *database = [FMDatabase databaseWithPath:dbPath];
    if (![database open]) {
